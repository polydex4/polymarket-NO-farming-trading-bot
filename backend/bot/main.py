...
from bot.exchange_holder import ExchangeHolder, ExchangeProxy
from bot.exchange.paper import PaperExchangeClient
from bot.demo_mode import demo_balance, is_demo_mode
...
def _build_exchange(exchange_cfg):
    ...
    initial = demo_balance() if is_demo_mode() else 100.0
    return PaperExchangeClient(initial_collateral_balance=initial)
...
async def run():
    ...
    exchange_holder = ExchangeHolder(exchange)
    exchange_proxy = ExchangeProxy(exchange_holder)

    def _reload_runtime_from_settings() -> None:
        try:
            new_exchange_cfg, _ = load_nothing_happens_config()
            new_exchange = _build_exchange(new_exchange_cfg)
            exchange_holder.set(new_exchange)
            logger.info(
                "runtime_reloaded_from_settings",
                extra={"live_send_enabled": new_exchange_cfg.live_send_enabled},
            )
        except Exception as exc:
            logger.error("runtime_reload_failed: %s", exc)

    get_settings_manager().on_reload(_reload_runtime_from_settings)
    ...
    api_port = os.getenv("API_PORT") or os.getenv("PORT") or os.getenv("DASHBOARD_PORT", "8080")
    api_task = None
    if api_port:
        ...
        api = BotApiServer(
            ...
            port=int(api_port),
            exchange=exchange,
            exchange_holder=exchange_holder,
        )
        api_task = asyncio.create_task(api.run(), name="api")
        logger.info("api_starting", extra={"port": int(api_port)})
    ...
    def _sync_signal_handler(*_args) -> None:
        loop.call_soon_threadsafe(request_shutdown)

    try:
        for sig_name in ("SIGINT", "SIGTERM"):
            if hasattr(signal, sig_name):
                loop.add_signal_handler(getattr(signal, sig_name), request_shutdown)
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if exchange_cfg.live_send_enabled:
            await asyncio.to_thread(exchange_proxy.bootstrap_live_trading, None)
            logger.info("exchange_bootstrapped")
            if risk.cfg.max_daily_drawdown_usd > 0:
                try:
                    startup_balance = await asyncio.to_thread(exchange_proxy.get_collateral_balance)
